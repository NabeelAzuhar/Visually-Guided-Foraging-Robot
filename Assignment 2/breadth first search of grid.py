...
def get_turn_direction(prev_direction, new_direction):
    if is_opposite(prev_direction, new_direction): 
        return "Error: Cannot turn 180 degrees" 

    # Handle backtracking
    if prev_direction[0] == new_direction[0] or prev_direction[1] == new_direction[1]:  
        return "RIGHT 45"  # Or "LEFT 45" - Choose a consistent direction for backtracking turns

    # Calculate relative angles (for other turns)
    angle_diff = ((new_direction[0] - prev_direction[0]), (new_direction[1] - prev_direction[1])) 

    if angle_diff in [(0, 1), (-1, 1), (-1, 0)]:  
        return "LEFT 45"
    elif angle_diff in [(0, -1), (1, -1), (1, 0)]: 
        return "RIGHT 45"
    else: 
